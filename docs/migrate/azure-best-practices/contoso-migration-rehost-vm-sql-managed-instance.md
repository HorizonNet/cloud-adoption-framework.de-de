...
description: Es wird beschrieben, wie Contoso eine lokale Anwendung mithilfe von Azure SQL Managed Instance auf Azure-VMs neu hostet.
author: givenscj
ms.author: abuck
ms.date: 07/01/2020
ms.topic: conceptual
...
In diesem Artikel wird veranschaulicht, wie das fiktive Unternehmen Contoso eine zweischichtige .NET-Front-End-Anwendung für Windows, die auf VMware-VMs ausgeführt wird, mit dem Azure Migrate-Dienst zu einer Azure-VM migriert. Außerdem wird erläutert, wie Contoso die Anwendungsdatenbank zu Azure SQL Managed Instance migriert.
...
- Die Anwendungsdatenbank (`SmartHotelDB`) soll zu Azure SQL Managed Instance migriert werden.
...
Im Rahmen des Lösungsentwurfs hat Contoso einen Featurevergleich zwischen Azure SQL-Datenbank und SQL Managed Instance durchgeführt. Hierbei haben die folgenden Faktoren Contoso dabei unterstützt, sich für SQL Managed Instance zu entscheiden.
...
- SQL Managed Instance unterstützt den SQL Server-Agent, eine wichtige Komponente der SmartHotel360-Anwendung. Contoso benötigt diese Kompatibilität, weil ansonsten die für diese Anwendung benötigten Wartungspläne umgestaltet werden müssen.
- Dank Software Assurance kann Contoso seine vorhandenen Lizenzen mit dem Azure-Hybridvorteil für SQL Server zu ermäßigten Preisen gegen SQL Managed Instance austauschen. Auf diese Weise kann Contoso bei SQL Managed Instance bis zu 30 % an Kosten einsparen.
...
2. Die Datenschicht wird mithilfe von Azure Database Migration Service migriert. Der Dienst stellt über eine Site-to-Site-VPN-Verbindung zwischen dem Contoso-Rechenzentrum und Azure eine Verbindung mit der lokalen SQL Server-VM her. Der Dienst migriert dann die Datenbank.
...
| --- | --- | --- |
| [Azure Database Migration Service](https://docs.microsoft.com/azure/dms/dms-overview) | Azure Database Migration Service ermöglicht die nahtlose Migration von mehreren Datenbankquellen zu Azure-Datenplattformen bei minimaler Ausfallzeit. | Informieren Sie sich über [unterstützte Regionen](https://docs.microsoft.com/azure/dms/dms-overview#regional-availability) und | [Preisen für Azure Database Migration Service](https://azure.microsoft.com/pricing/details/database-migration). |
| [Verwaltete Azure SQL-Datenbank-Instanz](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance) | SQL Managed Instance ist ein verwalteter Datenbankdienst, der eine vollständig verwaltete SQL Server-Instanz in der Azure-Cloud darstellt. Sie nutzt den gleichen Code wie die neueste Version der Microsoft SQL Server-Datenbank-Engine und verfügt über die neuesten Features, Leistungsverbesserungen und Sicherheitspatches. | Bei Verwendung einer SQL Managed Instance-Instanz, die in Azure ausgeführt wird, fallen Gebühren je nach Kapazität an. Lesen Sie mehr zu den [Preisen für SQL Managed Instance](https://azure.microsoft.com/pricing/details/sql-database/managed). |
...
> [!div class="checklist"]
>
> - **Schritt 1: Vorbereiten einer SQL Managed Instance-Instanz** Contoso benötigt eine bereits vorhandene verwaltete Instanz, zu der die lokale SQL Server-Datenbank-Instanz migriert wird.
...
> - **Schritt 3: Vorbereiten von Azure für die Azure Migrate- Servermigration.** Sie fügen das Tool für die Servermigration zum entsprechenden Azure Migrate-Projekt hinzu.
...
## <a name="step-1-prepare-a-sql-managed-instance"></a>Schritt 1: Vorbereiten einer SQL Managed Instance-Instanz

Contoso benötigt ein Subnetz, das die folgenden Anforderungen erfüllt, um eine Azure SQL Managed Instance-Instanz einzurichten:
...
- Das Subnetz muss über eine benutzerdefinierte Routingtabelle verfügen. Nur die Route `0.0.0.0/0` (nächster Hop zum Internet) zugewiesen werden.
- Wenn ein optionales benutzerdefiniertes DNS für das virtuelle Netzwerk angegeben wird, muss die virtuelle IP-Adresse `168.63.129.16` für die rekursiven Resolver in Azure zur Liste hinzugefügt werden. Hier erfahren Sie, wie Sie [ein benutzerdefiniertes DNS für Azure SQL Managed Instance-Instanz konfigurieren](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-custom-dns).
...
- Das Subnetz muss mindestens 16 IP-Adressen aufweisen. Informieren Sie sich, wie Sie [die Größe des Subnetzes für verwaltete Instanzen festlegen](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-vnet-configuration).
...
1. Sie erstellen ein neues virtuelles Netzwerk (`VNET-SQLMI-EU2`) in der primären Region (`East US 2`). Das virtuelle Netzwerk wird der Ressourcengruppe `ContosoNetworkingRG` hinzugefügt.
...
3. Dem Netzwerk werden zwei Subnetze hinzugefügt:
    ...
    - `SQLMI-SAW-EUS2` (`10.235.0.128/29`) Dieses Subnetz wird verwendet, um ein Verzeichnis an die verwaltete Instanz anzufügen.
      ...
      ![SQL Managed Instance: Virtuelles Netzwerk erstellen](./media/contoso-migration-rehost-vm-sql-managed-instance/mi-vnet.png)
...
- Informieren Sie sich über das [Erstellen eines virtuellen Netzwerks für eine Azure SQL Managed Instance-Instanz](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-vnet-configuration).
...
- Die Routingtabelle enthält Regeln (Routen), mit denen angegeben wird, wie von der verwalteten Instanz gesendete Pakete im virtuellen Netzwerk weitergeleitet werden sollen.
...
2. Nach dem Bereitstellen der Routingtabelle (`MIRouteTable`) wird eine Route mit dem Adresspräfix `0.0.0.0/0` hinzugefügt, um die Anforderungen von SQL Managed Instance zu erfüllen. Die Option **Typ des nächsten Hops** wird auf **Internet** festgelegt.
...
Jetzt können die Contoso-Administratoren eine SQL Managed Instance-Instanz bereitstellen:

1. Da die verwaltete Instanz als Geschäftsanwendung dient, wird sie in der primären Region des Unternehmens (`East US 2`) bereitgestellt. Die verwaltete Instanz wird zur Ressourcengruppe `ContosoRG` hinzugefügt.
2. Für die Instanz wird ein Tarif, eine Computegröße und Speicher ausgewählt. Lesen Sie mehr zu den [Preisen für SQL Managed Instance](https://azure.microsoft.com/pricing/details/sql-database/managed).
...
3. Nach der Bereitstellung der verwalteten Instanz werden zwei neue Ressourcen in der Ressourcengruppe `ContosoRG` angezeigt:
    ...
    - die neue SQL Managed Instance-Instanz
...
- Sie müssen Database Migration Service die Berechtigung für den Zugriff auf Azure Storage erteilen, damit die Sicherungsdateien für die Migration einer Datenbank hochgeladen werden können. Um Zugriff auf Azure Storage bereitzustellen, wird ein Azure Blob Storage-Container erstellt. Außerdem wird ein SAS-URI für den Blob Storage-Container generiert.
- Erstellen eines Azure Database Migration Service-Projekts.

Anschließend werden die folgenden Schritte ausgeführt:

1. Die Administratoren registrieren den Anbieter für die Datenbankmigration unter dem Abonnement. ![Database Migration Service: Register](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-subscription.png)

2. Sie erstellen einen Blob Storage-Container. Contoso generiert einen SAS-URI, damit Azure Database Migration Service darauf zugreifen kann.

    ![Database Migration Service: SAS-URI generieren](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-sas.png)
...
4. Sie ordnen die Database Migration Service-Instanz dem Subnetz `PROD-DC-EUS2` des virtuellen Netzwerks `VNET-PROD-DC-EUS2` zu.
    ...
    - Für `VNET-PROD-EUS2` erfolgt ein Peering mit `VNET-HUB-EUS2`, und die Verwendung von Remotegateways ist zulässig. Mit der Option **Remotegateways verwenden** wird sichergestellt, dass die Instanz nach Bedarf kommunizieren kann.
        ...
        ![Database Migration Service: Netzwerk konfigurieren](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-network.png)

**Benötigen Sie weitere Hilfe?**

- Informieren Sie sich, wie Sie [Azure Database Migration Service einrichten](https://docs.microsoft.com/azure/dms/quickstart-create-data-migration-service-portal).
...
1. **Einrichten eines Netzwerks:** Contoso hat bereits ein Netzwerk eingerichtet, das verwendet werden kann für die Azure Migrate- Servermigration bei der [Bereitstellung der Azure-Infrastruktur](./contoso-migration-infrastructure.md).
    ...
    - Bei der SmartHotel360-Anwendung handelt es sich um eine Produktionsanwendung. Die VMs werden zum Azure-Produktionsnetzwerk (`VNET-PROD-EUS2`) in der primären Region (`East US 2`) migriert.
...
1. Einrichten eines Netzwerks – Contoso hat bereits ein Netzwerk eingerichtet, das verwendet werden kann für die Azure Migrate- Servermigration bei der [Bereitstellung der Azure-Infrastruktur](./contoso-migration-infrastructure.md).
    ...
    - Bei der SmartHotel360-Anwendung handelt es sich um eine Produktionsanwendung. Die VMs werden zum Azure-Produktionsnetzwerk (`VNET-PROD-EUS2`) in der primären Region (`East US 2`) migriert.
    - Beide VMs werden in der Ressourcengruppe `ContosoRG` bereitgestellt, die für Produktionsressourcen verwendet wird.
    - Die Front-End-VM der Anwendung (`WEBVM`) wird zum Front-End-Subnetz (`PROD-FE-EUS2`) im Produktionsnetzwerk migriert.
    - Die Anwendungsdatenbank-VM (`SQLVM`) wird zum Datenbanksubnetz (`PROD-DB-EUS2`) im Produktionsnetzwerk migriert.

2. Bereitstellen der Azure Migrate-Appliance:
...
Lesen Sie mehr über das Einrichten der [Azure Migrate-Appliance](https://docs.microsoft.com/azure/migrate/migrate-services-overview#azure-migrate-server-migration-tool).
...
2. Sie wählen die zu migrierende Datenbank (`SmartHotel.Registration`) aus:

    ![Database Migration Service: Auswählen von Quelldatenbanken](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-wizard-source-db.png)
...
4. Unter **Neue Aktivität** > **Migration ausführen** geben sie die Einstellungen für die Ausführung der Migration an:
    ...
    - Der SAS-URI, mit dem für Azure Database Migration Service der Zugriff auf den Speicherkontocontainer gewährt wird, in den der Dienst die Sicherungsdateien für die Migration hochlädt
        ...
        ![Database Migration Service: Konfigurieren von Migrationseinstellungen](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-migration-settings.png)
...
7. Nach Abschluss der Migration überprüfen sie, ob die Zieldatenbanken auf der verwalteten Instanz vorhanden sind.

    ![Database Migration Service: Datenbankmigration überprüfen](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-monitor2.png)
...
5. Nach dem Neustart von IIS nutzt die Anwendung die Datenbank, die auf der SQL Managed Instance-Instanz ausgeführt wird.
...
Nach Abschluss der Migration wird die SmartHotel360-Anwendung auf einer Azure-VM ausgeführt, und die SmartHotel360-Datenbank befindet sich in der Azure SQL Managed Instance-Instanz.
...
Das Contoso-Sicherheitsteam überprüft die Azure-VMs und die SQL Managed Instance auf eventuelle Sicherheitsprobleme bei der Implementierung:

- Das Team überprüft die Netzwerksicherheitsgruppen, die zum Steuern des Zugriffs für den virtuellen Computer verwendet werden. Mit Netzwerksicherheitsgruppen kann sichergestellt werden, dass nur Datenverkehr, der für die Anwendung zulässig ist, erfolgreich ist.
...
- Das Team aktiviert die Bedrohungserkennung auf der verwalteten Instanz. Bei der Bedrohungserkennung wird eine Benachrichtigung mit dem Hinweis, dass ein Ticket erstellt werden soll, an das Sicherheitsteam von Contoso bzw. das Service Desk-System gesendet. Lesen Sie mehr zur [Bedrohungserkennung für SQL Managed Instance](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-threat-detection).
     ...
     ![Sicherheit in SQL Managed Instance: Bedrohungserkennung](./media/contoso-migration-rehost-vm-sql-managed-instance/mi-security.png)
...
- Schützen von Daten: Contoso sichert die Daten auf den VMs mithilfe des Azure Backup-Diensts. Weitere Informationen finden Sie unter [Ein Überblick über die Sicherung von Azure-VMs](https://docs.microsoft.com/azure/backup/backup-azure-vms-introduction).
- Aufrechterhalten des Anwendungsbetriebs: Contoso repliziert die Anwendungs-VMs in Azure mithilfe von Site Recovery in einer sekundären Region. [Weitere Informationen](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-quickstart)
...
In diesem Artikel hostet Contoso die SmartHotel360-Anwendung in Azure neu, indem das Unternehmen die Front-End-VM der Anwendung mithilfe des Azure Migrate-Diensts zu Azure migriert. Contoso migriert die lokale Datenbank zu einer Azure SQL Managed Instance-Instanz, indem Database Migration Service von Azure verwendet wird.
