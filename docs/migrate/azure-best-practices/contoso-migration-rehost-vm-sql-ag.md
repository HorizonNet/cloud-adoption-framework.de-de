...
- Contoso möchte nicht in diese Anwendung investieren. Die Investition ist wichtig für das Geschäft, in ihrer derzeitigen Form soll die App jedoch lediglich sicher in die Cloud verschoben werden.
...
- Contoso migriert das Anwendungs-Front-End `WEBVM` zu einer Azure-IaaS-VM.
...
Im Rahmen des Lösungsentwurfs hat Contoso einen Featurevergleich zwischen Azure SQL-Datenbank und SQL Server durchgeführt. Hierbei haben die folgenden Überlegungen Contoso dabei unterstützt, sich für eine Azure-IaaS-VM zu entscheiden, auf der SQL Server ausgeführt wird:
...
| [Azure Migrate](https://docs.microsoft.com/azure/migrate/migrate-overview) | Contoso nutzt den Azure Migrate-Dienst, um seine VMware-VMs zu bewerten. Azure Migrate bewertet die Eignung der Computer für die Migration. Der Dienst stellt Schätzungen zur Größe und zu den Kosten für die Ausführung in Azure bereit. | Azure Migrate ist ohne Aufpreis erhältlich. Es können jedoch durch die Tools (Originalanbieter oder ISV), die Sie zur Bewertung und Migration verwenden, Gebühren anfallen. Weitere Informationen zu den Preisen von Azure Migrate finden Sie [hier](https://azure.microsoft.com/pricing/details/azure-migrate). |

## <a name="migration-process"></a>Migrationsprozess

Contoso-Administratoren migrieren die Anwendungs-VMs zu Azure.
...
- Sobald die Datenbank überprüft wurde, wird sie mithilfe von Azure Database Migration Service zu einem SQL Server-Cluster in Azure migriert.
...
> - **Schritt 9: Migrieren der VMs über Azure Migrate.** Führen Sie eine Testmigration aus, um sicherzustellen, dass alles wie erwartet funktioniert. Führen Sie dann eine Migration zu Azure aus.
...
2. Unter **Assistent zum Erstellen virtueller Maschinen** > **Grundlagen** konfiguriert das Unternehmen Folgendes:
    ...
    - Das Unternehmen stellt die VMs in der primären Region (`East US 2`) in der Ressourcengruppe `ContosoRG` bereit.

3. Unter **Größe** beginnt das Unternehmen mit `D2S v3`-Instanzen für beide VMs. Nach Bedarf kann die Skalierung zu einem späteren Zeitpunkt durchgeführt werden.
...
- Hilfe zur [Bereitstellung einer SQL Server-VM](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-server-provision#1-configure-basic-settings).
...
1. Das Unternehmen richtet ein Azure Storage-Konto ein, um als Cloudzeuge zu fungieren.
...
3. Contoso platziert das Konto in einer dritten Region (`South Central US`). Es platziert das Konto außerhalb der primären und sekundären Region, damit es während eines regionalen Ausfalls verfügbar bleibt.
...
Contoso-Administratoren können jetzt Always On-Verfügbarkeitsgruppen aktivieren:
...
2. Das Unternehmen platziert den Lastenausgleich im Datenbanksubnetz (`PROD-DB-EUS2`) des Produktionsnetzwerks (`VNET-PROD-EUS2`).
3. Dem Lastenausgleich wird eine statische IP-Adresse (`10.245.40.100`) zugewiesen.
...
Contoso-Administratoren erstellen einen Integritätstest, damit der Lastenausgleich den Status der Anwendung überwachen kann. Abhängig von der Reaktion auf Integritätstests werden durch den Integritätstest dynamisch VMs zur Lastenausgleichsrotation hinzugefügt oder daraus entfernt.
...
4. Es aktiviert Floating IP (Direct Server Return). Dies ist für SQL Server Always On immer erforderlich.
...
- Ein VNET, in dem sich VMs befinden, wenn sie migriert werden.
...
2. Contoso-Administratoren erstellen ein Azure Storage-Konto (`contosovmsacc20180528`) in der primären Region.
...
9. Geben Sie unter **Datenträger** an, ob die VM-Datenträger in Azure repliziert werden sollen, und wählen Sie den Datenträgertyp (SSD Standard/HDD Standard oder Managed Disks Premium) in Azure aus. Wählen Sie **Weiter**aus.
...
Die Administratoren von Contoso führen die Migration mithilfe von Azure Database Migration Service anhand des [Schritt-für-Schritt-Tutorials für die Migration](https://docs.microsoft.com/azure/dms/tutorial-sql-server-azure-sql-online) durch. Sie können Online-, Offline- und Hybridmigrationen (Vorschauversion) ausführen.

Zusammenfassend müssen Sie die folgenden Schritte ausführen:

- Erstellen Sie eine Azure Database Migration Service-Instanz, die mit dem VNET verbunden ist. Verwenden Sie dazu den Premium-Tarif.
...
Wenn die Anwendungsdatenbank auf `SQLAOG1` ausgeführt wird, können Contoso-Administratoren diese jetzt mithilfe von AlwaysOn-Verfügbarkeitsgruppen schützen. Das Unternehmen konfiguriert SQL Server Always On mit SQL Server Management Studio und weist anschließend über das Windows-Clustering einen Listener zu.
...
2. Unter **Optionen angeben** gibt das Unternehmen der Verfügbarkeitsgruppe den Namen `SHAOG`. Unter **Datenbanken auswählen** wählt es die `SmartHotel360`-Datenbank aus.
...
Das Ausführen einer Testmigration dient zur Sicherstellung, dass vor der Migration alles wie erwartet funktioniert.

1. Contoso führt ein Testfailover auf letzten verfügbaren Zeitpunkt aus (`Latest processed`).
...
3. Nachdem IIS neu gestartet wurde, verwendet die Anwendung nun die Datenbank, die auf der verwalteten Instanz ausgeführt wird.
...
Nach der Migration wird die SmartHotel360-Anwendung auf einer Azure-VM ausgeführt, und die SmartHotel360-Datenbank befindet sich im Azure SQL-Cluster.
...
Das Sicherheitsteam von Contoso überprüft die virtuellen Computer `WEBVM`, `SQLAOG1` und `SQLAOG2` auf eventuell vorhandene Sicherheitsprobleme.
...
- Contoso repliziert die Anwendungs-VMs in Azure mithilfe von Site Recovery in eine sekundäre Region, um Anwendungen funktionstüchtig zu halten. [Weitere Informationen](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-quickstart)
...
- Contoso verwendet [Azure Cost Management und Abrechnung](https://docs.microsoft.com/azure/cost-management-billing/cost-management-billing-overview), um sicherzustellen, dass das von den IT-Führungskräften festgelegte Budget nicht überschritten wird.

## <a name="conclusion"></a>Zusammenfassung

In diesem Artikel wurde beschrieben, wie Contoso der SmartHotel360-Anwendung in Azure einen neuen Host zugewiesen hat, indem die Front-End-VM der Anwendung mithilfe des Azure Migrate-Diensts zu Azure migriert wurde. Contoso hat die Anwendungsdatenbank mithilfe von Azure Database Migration Service zu einem in Azure bereitgestellten SQL Server-Cluster migriert und in einer SQL Server Always On-Verfügbarkeitsgruppe geschützt.
