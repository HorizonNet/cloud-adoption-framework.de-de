...
Nach Möglichkeit können Sie einen von mehreren Diensten verwenden, um Wiederherstellungsaktionen zu automatisieren. Dazu gehören System Center Orchestrator, Azure Automation, Azure Logic Apps oder Autoskalierung bei flexiblen Workloads. Die Benachrichtigung der verantwortlichen Teams ist zwar die häufigste Maßnahme bei Warnungen, aber auch die Automatisierung von Korrekturmaßnahmen könnte geeignet sein. Dies kann dazu beitragen, den gesamten Incident Management-Prozess zu optimieren. Die Automatisierung dieser Wiederherstellungsaufgaben kann auch das Risiko von menschlichen Fehlern reduzieren.

## <a name="azure-monitor-alerting"></a>Azure Monitor-Warnungen

Wenn Sie ausschließlich Azure Monitor verwenden, gelten die folgenden Grundsätze. Beachten Sie diese Grundsätze, wenn Sie Geschwindigkeit, Kosten und Speichervolumen in Azure Monitor bewerten.

Abhängig vom Feature und der verwendeten Konfiguration gibt es sechs Repositorys, die Überwachungsdaten speichern.
...
- **Aktivitätsprotokollspeicher**: Wird für alle Aktivitätsprotokoll- und Dienstintegritätsereignisse verwendet. Eine dedizierte Warnungserstellung ist möglich. Dieser Speicher enthält Ereignisse auf Abonnementebene, die für Objekte in Ihrem Abonnement auftreten, wie sie von außen wahrgenommen werden. Beispielsweise wird beim Festlegen einer Richtlinie auf eine Ressource zugegriffen, oder eine Ressource wird gelöscht.
...
- **Event Hubs**: Wird im Allgemeinen verwendet, um Daten in lokale Tools oder Partnertools für Überwachung/ITSM zu streamen.
...
- [Metrikwarnung](/azure/azure-monitor/platform/alerts-metric): Warnungen zu Daten in der Azure Monitor-Metrikdatenbank. Warnungen werden ausgelöst, wenn ein Überwachungswert einen benutzerdefinierten Schwellwert überschreitet. Dann werden sie erneut ausgelöst, wenn der Wert in den Zustand „Normal“ zurückkehrt.

- [Protokollabfragewarnung](/azure/azure-monitor/platform/alerts-log-query): Verfügbar für Warnmeldungen zu Inhalten in den Application Insights- oder Azure-Protokollspeichern. Das Auslösen von Warnungen basierend auf arbeitsbereichsübergreifenden Abfragen ist ebenfalls möglich.

- [Aktivitätsprotokollwarnung](/azure/azure-monitor/platform/alerts-activity-log): Warnungen zu Elementen im Aktivitätsprotokollspeicher, ausgenommen sind Service Health-Daten.

- [Service Health-Warnung](/azure/azure-monitor/platform/alerts-activity-log-service-notifications?toc=%2fazure%2fservice-health%2ftoc.json): Ein besonderer Warnungstyp, der ausschließlich für Service Health-Probleme gilt, die aus dem Aktivitätsprotokollspeicher stammen.
...
**Importieren von lokalen Daten**: Wenn Sie versuchen, Abfragen über lokale und Azure-Computer hinweg zu stellen und zu überwachen, können Sie mit dem Log Analytics-Agent Daten des Gastbetriebssystems erfassen. Dann können Sie ein Feature namens [Protokolle zu Metriken](/azure/azure-monitor/platform/alerts-metric-logs) verwenden, um diese Metriken in den Metrikenspeicher zu integrieren. Bei dieser Methode wird ein Teil des Erfassungsvorgangs im Azure-Protokollspeicher umgangen und die Daten sind deshalb früher in der Metrikdatenbank verfügbar.
...
Beachten Sie die geltenden [Einschränkungen](/azure/azure-subscription-service-limits#azure-monitor-limits) für die Anzahl von Benachrichtigungen, die erstellt werden können. Einige Grenzwerte (aber nicht alle) können auf Wunsch erhöht werden. Wenden Sie sich hierzu an den Support.
