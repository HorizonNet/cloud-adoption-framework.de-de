...
Das Kostenmanagement im gesamten Unternehmen ist eine Funktion von Cloudgovernance und Cloudvorgängen. Alle Entscheidungen des Kostenmanagements bewirken jedoch eine Änderung der Ressourcen, die eine Workload unterstützen. Wirken sich diese Änderungen auf die Architektur einer Workload aus, sind weitere Überlegungen anzustellen, um die Auswirkungen auf Endbenutzer und Geschäftsfunktionen zu minimieren. Das Cloudeinführungsteam, das diese Workload konfiguriert oder entwickelt hat, ist wahrscheinlich dafür zuständig, diese Arten von Änderungen abzuschließen.

In diesem Artikel werden bewährte Methoden in zwei Kategorien unterteilt: Bewährte Betriebsmethoden und bewährte Methoden für Workloads. Governance-, Betriebs-und Einführungsteams sollten sich stets an Änderungen zur Kostenoptimierung orientieren; anhand der Unterteilung in diese beiden Kategorien kann jedoch veranschaulicht werden, in welchen Fällen die Abgrenzung der Pflichten eindeutig ist.

## <a name="operational-cost-management-best-practices"></a>Bewährte Methoden für das betriebliche Kostenmanagement

Die folgenden bewährten Methoden werden in der Regel von einem Mitglied des Cloudgovernance- oder Cloudbetriebsteams in Einklang mit Patching- und anderen geplanten Wartungsvorgängen befolgt. Jeder dieser bewährten Methoden wird später in diesem Artikel ein handlungsrelevanter Leitfaden zugeordnet.

- **Kennzeichnung (Tagging) ist für jede Art von Governance von entscheidender Bedeutung:** Achten Sie darauf, dass bei allen Workloads und Ressourcen **[ordnungsgemäße Benennungs- und Kennzeichnungskonventionen](../../ready/azure-best-practices/naming-and-tagging.md)** eingehalten werden, und [setzen Sie Kennzeichnungskonventionen mithilfe von Azure-Richtlinien durch](https://docs.microsoft.com/azure/governance/policy/tutorials/govern-tags).
- **Bestimmen von Gelegenheiten zur richtigen Dimensionierung:** Untersuchen Sie Ihre derzeitige Ressourcenauslastung und Leistungsanforderungen für die gesamte Umgebung, um Ressourcen zu ermitteln, die über einen längeren Zeitraum (im Allgemeinen länger als 90 Tage) nicht ausgelastet waren.
- **Richtige Dimensionierung bereitgestellter SKUs:** Ändern Sie jede nicht ausgelastete Ressource so, dass sie die kleinste Instanz oder SKU verwendet, die die Leistungsanforderungen der jeweiligen Ressource unterstützen kann.
- **Automatisches Herunterfahren von VMs:** Wird eine VM nicht durchgehend verwendet, sollten Sie das automatische Herunterfahren in Erwägung ziehen. Die VM wird nicht gelöscht und ihre Bereitstellung nicht aufgehoben, aber sie verursacht keine Compute- und Speicherkosten mehr, bis sie wieder eingeschaltet wird.
- **Automatisches Herunterfahren aller Nicht-Produktionsressourcen**: Wenn eine VM zu einer Nicht-Produktionsumgebung gehört (insbesondere gilt dies für Entwicklungsumgebungen), richten Sie eine Richtlinie zum automatischen Herunterfahren ein, um die Kosten für die unzureichende Auslastung zu senken. Nutzen Sie nach Möglichkeit Entwicklungs- und Testumgebungen für Self-Service-Optionen, um die Eigenverantwortlichkeit der Entwickler für Kosten zu fördern.
...
- **Anpassen von Parametern für die automatische Skalierung:** Bei der operativen Überwachung werden wahrscheinlich Verwendungsmuster für diverse Ressourcen aufgedeckt. Werden diese Verwendungsmuster den Parametern zum Fördern der automatischen Skalierung zugeordnet, passt das Betriebsteam Parameter für die automatische Skalierung häufig gemäß dem saisonalen Bedarf und/oder Änderungen von Budgetzuteilungen an. * Wichtige Vorsichtsmaßnahmen werden unter den bewährten Methoden für das Kostenmanagement von Workloads erläutert.

## <a name="workload-cost-management-best-practices"></a>Bewährte Methoden für das Kostenmanagement von Workloads

Bevor Sie Änderungen der Architektur vornehmen, sollten Sie sich mit dem technischen Leiter für die Workload besprechen. Es sollte eine Überprüfung der Workload mit der [Azure-Architekturüberprüfung](/assessments/?id=azure-architecture-review) und dem [Azure Architecture Framework](/azure/architecture/framework/) ausgeführt werden, um fundierte Entscheidungen zu den folgenden Architekturänderungen treffen zu können.

- **Azure App Services.** Überprüfen Sie die Produktionsanforderungen für alle Premium-App Service-Pläne. Ohne ein Verständnis der geschäftlichen Anforderungen für eine Workload und der Konfiguration der zugrunde liegenden Ressourcen kann nur schwer festgestellt werden, ob ein Premium-App Service-Plan erforderlich ist.
...
- **Azure Migrate:** Um Kosten zu schätzen, müssen Sie alle Ressourcen überprüfen und berücksichtigen, die für die Ausführung Ihrer Workloads in Azure erforderlich sind. Um diese Daten zu sammeln, erstellen Sie ein Inventar Ihrer Ressourcen, einschließlich Servern, virtueller Computer, Datenbanken und Speicher. Sie können Azure Migrate verwenden, um diese Informationen zu sammeln.
  ...
  - Eine Azure Migrate-Bewertung enthält geschätzte Kosten.
    - **Computekosten:** Wenn Sie eine Bewertung erstellen, berechnet Azure Migrate anhand der empfohlenen Azure-VM-Größe mithilfe der Abrechnungs-API die geschätzten monatlichen Kosten für den virtuellen Computer. Bei der Schätzung werden Betriebssystem, Software Assurance, reservierte Instanzen, VM-Betriebszeit, Standort und Währungseinstellungen berücksichtigt. Die Kosten aller virtuellen Computer werden in der Bewertung zusammengefasst, um so die monatlichen Compute-Gesamtkosten zu berechnen.
...
- Verschaffen Sie sich einen [Überblick über Azure Migrate](https://docs.microsoft.com/azure/migrate/migrate-overview).
...
**Speicheroptimiert** | Datenträgerdurchsatz und -E/A auf hohem Niveau. | Geeignet für Big Data sowie SQL- und NoSQL-Datenbanken.
...
- [Weitere Informationen zum](https://docs.microsoft.com/azure/storage/common/storage-import-export-service) Azure Import/Export für die Übernahme großer Datenmengen in Azure-Blobs und Azure Files.
- [Vergleichen Sie](https://docs.microsoft.com/azure/storage/common/storage-introduction?toc=/azure/storage/blobs/toc.json) Datentypen für Blobs, Dateien und Datenträgerspeicher.
...
- Bestimmen Sie Verwendungsmuster, was Ihnen die Möglichkeit gibt, die Größe von Ressourcen zu verringern und/oder sie anzuhalten/zu beenden, um Ihre Kosten weiter zu senken.

Bewährte Methoden in diesem Abschnitt umfassen die Nutzung von Azure-Hybridvorteilen und reservierten VMs, das Senken von Cloudausgaben über alle Abonnements, die Verwenden von Azure Cost Management für die Budgetierung und Analyse von Kosten, die Überwachung von Ressourcen und die Implementierung von Budgets für Ressourcengruppen sowie die Optimierung der Überwachung, von Speicher und VMs.
...
![Reservierte Instanzen](../../migrate/azure-best-practices/media/migrate-best-practices-costs/reserve.png)
...
- Erfahren Sie, wie Sie [Kosten anhand von Empfehlungen optimieren](https://docs.microsoft.com/azure/cost-management-billing/costs/tutorial-acm-opt-recommendations?toc=/azure/billing/toc.json) und [unerwartete Gebühren vermeiden](https://docs.microsoft.com/azure/billing/billing-getting-started).
...
## <a name="best-practice-reduce-non-production-costs"></a>Bewährte Methode: Senken nicht produktionsbezogener Kosten

Während der Entwicklungszyklen werden Umgebungen für Entwicklung, Tests und Qualitätssicherung (QS) benötigt. Leider kommt es häufig vor, dass diese Umgebungen noch lange nach dem Ende ihrer Nutzung bereitgestellt bleiben. Eine regelmäßige Überprüfung der nicht verwendeten nicht produktionsbezogenen Umgebungen kann sich unmittelbar auf die Kosten auswirken.

Ziehen Sie darüber hinaus allgemeine Kosteneinsparungen für alle nicht produktionsbezogenen Umgebungen in Betracht:

- Stufen Sie nicht in der Produktion verwendete Ressourcen auf günstigere VMs der B-Serie und Standardspeicher herab.
- Wenden Sie Azure-Richtlinien an, um Kostensenkungen für alle nicht in der Produktion eingesetzten Ressourcen auf Ressourcenebene vorzuschreiben.

**Weitere Informationen**:

- [Verwenden Sie Tags](https://docs.microsoft.com/azure/azure-resource-manager/management/tag-resources), um Entwicklungs-, Test- oder QS-Ziele zum Ändern der Dimensionierung und/oder Beendigung zu kennzeichnen.
- [Automatisches Herunterfahren von VMs](https://docs.microsoft.com/azure/cost-management-billing/manage/getting-started#consider-cost-cutting-features-like-auto-shutdown-for-vms) legt eine nächtliche Beendigungszeit für VMs fest. Die Verwendung dieser Funktion beendet nicht in der Produktion eingesetzte VMs jede Nacht, was Entwickler dazu zwingt, diese VMs neu zu starten, wenn sie zur Fortsetzung der Entwicklungsarbeit bereit sind.
...
- **Eine Kostenanalyse durchführen:** Stellen Sie eine Kostenanalyse auf, um Ihre Organisationskosten zu untersuchen und zu analysieren, damit Sie besser verstehen, wie Kosten anfallen, und Ausgabentrends erkennen können.
  ...
  - Sie können eine Kostenanalyse abrufen, die Gesamtkosten für den aktuellen Monat und die akkumulierten täglichen Kosten anzeigt.
    ...
    ![Cost Management-Analyse](../../migrate/azure-best-practices/media/migrate-best-practices-costs/analysis.png)
