...
![Identitäts- und Zugriffsverwaltung](./media/iam.png)

_Abbildung 1: Identitäts- und Zugriffsverwaltung._

**Überlegungen zum Entwurf:**

- Beim Aufstellen eines Frameworks zu IAM und Governance gibt es Grenzen hinsichtlich der Anzahl von benutzerdefinierten Rollen und Rollenzuweisungen, die berücksichtigt werden müssen. Diese finden Sie hier: [Grenzwerte für rollenbasierte Zugriffssteuerung (Role-Based Access Control, RBAC)](https://docs.microsoft.com/azure/azure-resource-manager/management/azure-subscription-service-limits#role-based-access-control-limits)
...
- Ressourcenbesitz: zentralisiert oder im Verbund
  ...
  - Gemeinsam genutzte Ressourcen oder jeder Aspekt der Umgebung, der eine Sicherheitsgrenze implementiert oder erzwingt, wie z. B. das Netzwerk, müssen zentral verwaltet werden. Dies ist sowohl eine Anforderung vieler regulatorischer Bestimmungen als auch eine Standardpraxis für jede Organisation, die Zugriff auf vertrauliche oder kritische Geschäftsressourcen gewährt oder verweigert.
...
- Nutzen Sie nach Möglichkeit Azure AD [RBAC](https://docs.microsoft.com/azure/role-based-access-control/overview), um den Zugriff auf Ressourcen auf Datenebene zu verwalten (z. B. Azure Key Vault, Speicherkonto oder SQL-Datenbank-Instanz).
...
- Erzwingen Sie für alle Benutzer mit Zugriffsrechten für die Azure-Umgebungen eine mehrstufige Authentifizierung. Dies ist eine Anforderung vieler Compliancebestimmungen und senkt das Risiko des Diebstahls von Anmeldeinformationen und des unberechtigten Zugriffs erheblich.

- Setzen Sie [Azure AD Privileged Identity Management (PIM)](https://docs.microsoft.com/azure/active-directory/privileged-identity-management/pim-configure) ein, um einen ständigen Zugriff zu unterbinden und das Prinzip der geringsten Rechte umzusetzen. Ordnen Sie die Rollen Ihrer Organisation dem erforderlichen Mindestzugriff zu. Azure AD PIM kann entweder als Erweiterung vorhandener Tools und Prozesse dienen, wie oben beschrieben native Azure-Tools nutzen oder beides nach Bedarf nutzen.
...
- Nutzen Sie Azure AD PIM-Zugriffsüberprüfungen, um Ressourcenberechtigungen regelmäßig zu prüfen. Zugriffsüberprüfungen sind Teil vieler Compliancebestimmungen, weshalb viele Organisationen bereits über ein Verfahren zur Erfüllung dieser Anforderung verfügen.

- Integrieren Sie Azure AD-Protokolle in [Azure Monitor](https://docs.microsoft.com/azure/active-directory/reports-monitoring/concept-activity-logs-azure-monitor), das für die Plattform von zentraler Bedeutung ist. Azure Monitor ermöglicht eine einzige Quelle für die Wahrheit für Protokoll- und Überwachungsdaten in Azure und gibt Unternehmen cloudnative Optionen an die Hand, um die Anforderungen an Sammlung und Aufbewahrung von Protokollen zu erfüllen.
...
- Anwendungen, die sich auf Domänendienste stützen und ältere Protokolle verwenden, können [Azure AD Domain Services](https://docs.microsoft.com/azure/active-directory-domain-services) einsetzen.
...
- Wenn es in einer Organisation ein Szenario gibt, in dem auf eine Anwendung mit integrierter Windows-Authentifizierung remote über Azure AD zugegriffen werden muss, sollten Sie [Azure AD-Anwendungsproxy](https://docs.microsoft.com/azure/active-directory/manage-apps/application-proxy) verwenden.

- Es besteht ein Unterschied zwischen Azure AD, Azure AD Domain Services und AD DS, das unter Windows Server ausgeführt wird. Beurteilen Sie Ihre Anwendungsbedürfnisse, und ermitteln und dokumentieren Sie den jeweils verwendeten Authentifizierungsanbieter. Planen Sie für alle Anwendungen entsprechend.

- Beurteilen Sie die Kompatibilität von Workloads für AD DS unter Windows Server und für Azure AD Domain Services.
...
- Stellen Sie Azure AD Domain Services innerhalb der primären Region bereit, da dieser Dienst nur in ein Abonnement aufgenommen werden kann.

- Nutzen Sie für die Authentifizierung bei Azure-Diensten verwaltete Identitäten anstelle von Dienstprinzipalen. Dadurch sinkt das Risiko des Diebstahls von Anmeldeinformationen.
